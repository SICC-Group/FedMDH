...
func (app *Application) StartTask_FedMDH(w http.ResponseWriter, r *http.Request) {
	...
	http.Post("http://127.0.0.1:8000/api/v1/task/create/", "application/json", bytes.NewReader(task_body))
	// 以下在python后端进行多用户训练
	response1, _ := http.Post("http://127.0.0.1:8000/api/v1/fl/startTrain_fedmdh/"+task.ID, "application/json", bytes.NewReader(task_body))
	defer func(Body io.ReadCloser) {
		err := Body.Close()
		if err != nil {
			print("ERROR")
...
// 获取FedMDH任务中单个组织的训练结果
func (app *Application) GetSingleResult_FedMDH(w http.ResponseWriter, r *http.Request) {
	setupCORS(&w)
	if r.Method == "OPTIONS" {
		fmt.Println("from AddMgeData, 处理跨域options请求")
		return
	...
	orgName, _ := data["orgName"].(string)
	taskID, _ := data["taskID"].(string)
	response, _ := http.Get("http://127.0.0.1:8000/api/v1/fl/get_single_result_fedmdh/" + taskID + "/" + orgName)
	defer func(Body io.ReadCloser) {
		err := Body.Close()
		if err != nil {
			print("ERROR")
...
// 获取联邦任务的训练结果
func (app *Application) GetMultiResult_FedMDH(w http.ResponseWriter, r *http.Request) {
	setupCORS(&w)
	// db.CreateEmptyTable()
	if r.Method == "OPTIONS" {
		fmt.Println("from GetTemplateByID, 处理跨域options请求")
	...
	taskID := r.URL.Query().Get("taskID")
	response, _ := http.Get("http://127.0.0.1:8000/api/v1/fl/get_multi_result_fedmdh/" + taskID)
	defer func(Body io.ReadCloser) {
		err := Body.Close()
		if err != nil {
			print("ERROR")
