...
export const _Trainresults: FC<RouteComponentProps & WithSession> = (props) => {
    ...
    const [tdata, setTdata] = useState({
        ...
        "shap_value_bar_pic": "/img/accepted.png",
        "shap_value_dot_pic": "/img/accepted.png",
        "y_test_vs_y_pred_pic": "/img/accepted.png"
    });
    ...
    useEffect(() => {
        ...
        if (taskInfo.fedMethod === "FedAvg") {
            FLApiFetch(`http://localhost:9001/api/v1/task/getMultiResult?taskID=${taskID}`).then((value) => {
                console.log(value);
                setTdata(value);
            })
        } else if (taskInfo.fedMethod === "FedMDH"){
            FLApiFetch(`http://localhost:9001/api/v1/task/getMultiResult_FedMDH?taskID=${taskID}`).then((value) => {
                console.log(value);
                setTdata(value);
            })
        }
            ...
            <Card title={`Training Results - ${taskInfo?.Name}`} bordered={false}>
                ...
                <Tabs defaultActiveKey="1">
                    <TabPane tab="Scatter plot of prediction results" key="1">
                        <img
                            width="40%"
                            src={data == null ? '' : data.images.img1}
                    ...
                    <TabPane tab="Feature importance dot plot" key="2">
                        <img
                            width="40%"
                            src={data == null ? '' : data.images.img2}
                    ...
                    <TabPane tab="Loss function convergence curve" key="3">
                        <img
                            width="40%"
                            src={data == null ? '' : data.images.img3}
                            // src={data == null ? '' : "/img/accepted.png"}
