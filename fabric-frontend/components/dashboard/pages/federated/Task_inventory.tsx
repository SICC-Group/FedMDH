...
export const _TaskInventory: FC<RouteComponentProps & WithSession> = (props) => {
    ...
    const handleStartTraining = (taskId: string) => {
        // 这里实现开始训练的逻辑
        FLApiFetch(`http://localhost:9001/api/v1/task/getTask?taskID=${taskId}`).then((value) => {
            if (value.fedMethod === 'FedAvg') {
                ...
                notification.success({
                    ...
                    description: <FormattedMessage id='has_started' defaultMessage="您的FedAvg任务已经开始训练。" />,
                });
            }else if (value.fedMethod === 'FedMDH') {
                FLApiFetch(`http://localhost:9001/api/v1/task/startTrain_FedMDH?taskID=${taskId}`, 'POST')
                notification.success({
                    message: <FormattedMessage id='Submitted_successfully' defaultMessage="任务名称" />,
                    description: <FormattedMessage id='has_started' defaultMessage="您的FedMDH任务已经开始训练。" />,
                });
            }
        })
