...
from apps.fl.FedMDH.main import FEDMDH_PROJECT_PATH, FEDMDH_PYTHON_PATH
...
def startTrain_multiuser(request, task_id):
    ...
    org_dataset_map = json.dumps(task.orgs_datasets,ensure_ascii=False)
    # p = subprocess.Popen([PYTHON_PATH,
    ...
    with subprocess.Popen([PYTHON_PATH,
        ...
        output, error = proc.communicate(timeout=180)
        print(output)
        print(error)
        proc.kill()
    task.task_status = 2
...
def startTrain_fedmdh(request, task_id):
    task = Task.objects.filter(task_id=task_id)[0]
    task.task_status = 1
    task.save()
    res_path = os.path.join(PROJECT_PATH, f"results/{task.task_id}")
    ...
    with subprocess.Popen([FEDMDH_PYTHON_PATH,
                           os.path.join(FEDMDH_PROJECT_PATH, "main.py"), f"--taskID={task_id}",
                           f"--accepted_orgs={accepted_orgs}", f"--org_dataset_map={org_dataset_map}",
                           f"--num_users={len(task.accepted_orgs)}",
                           f"--result_path={res_path}", f"--test_dataset={task.test_dataset}"],
                          stdout=subprocess.PIPE,
                          stderr=subprocess.PIPE, text=True) as proc:
    ...
    task.task_status = 2
    task.save()
    res_path = os.path.join(FEDMDH_PROJECT_PATH, f"results/{task.task_id}")
    res = {
        "predict_vs_actual": res_path + "/predicted_vs_actual.png",
        "test_data_loss": res_path + "/test_data_loss.png",
    }
    return JsonResponse(res)
...
@login_required_api
def get_single_result_fedmdh(request, task_id, username):
    task = Task.objects.filter(task_id=task_id)[0]
    if task.task_status < 2:
        return JsonResponse({"message": "任务未完成"})
    else:
        res_path = os.path.join(FEDMDH_PROJECT_PATH, f"results/{task.task_id}",f"user_{username}")
        res = {
            "loss_pic": res_path + "/test_loss_plot.png",
        }
        return JsonResponse(res)


@login_required_api
def get_multi_result_fedmdh(request, task_id):
    task = Task.objects.filter(task_id=task_id)[0]
    if task.task_status < 2:
        return JsonResponse({"message": "任务未完成"})
    else:
